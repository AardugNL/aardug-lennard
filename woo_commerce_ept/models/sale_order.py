...
class SaleOrder(models.Model):
    ...
    def _search_woo_order_ids(self, operator, value):
        query = """select so.id from stock_picking sp
                    ...
                    where sp.updated_in_woo %s %s and sp.state != 'cancel' and
                    so.woo_instance_id notnull""" % (operator, value)
    ...
    def create_woo_order_line(self, line_id, product, quantity, price, taxes, tax_included, woo_instance,
        ...
        rounding = bool(woo_instance.tax_rounding_method == 'round_per_line')
        line_vals = {
            "name": product.name,
    ...
    def _create_invoices(self, grouped=False, final=False, date=None):
        ...
        woo_orders = self.filtered(lambda x: x.woo_instance_id)
        orders = self - woo_orders
        if woo_orders:
            instances = woo_orders.woo_instance_id
            for instance in instances:
                instance_orders = self.filtered(lambda x: x.woo_instance_id == instance)
                rounding = bool(instance.tax_rounding_method == 'round_per_line')
                res += super(SaleOrder, instance_orders.with_context(round=rounding))._create_invoices(grouped, final,
                                                                                                       date)
        if orders:
            res += super(SaleOrder, orders)._create_invoices(grouped, final, date)
        return res
